...
RfastCor_wrapper <- function(x, Method = "pearson")
{
  gene_names    <- rownames(x)
  x             <- as.matrix(x)
  if(Method == "spearman"){ x <- Rfast::rowRanks(x); }
...
RfastTOMdist <- function(A)
{
  ...
  denomTOM    <- Rfast::Pmin(denomHelp,Rfast::transpose(denomHelp)) + (1-A); rm(denomHelp,kk); gc();

  #numTOM      <- eigenMapMatMult(A,A) + A; # Rfast::mat.mult appear to be unstable in current release, coded in Rcpp from stack exchange
  numTOM      <- Rfast::mat.mult(A, A) + A; rm(A); gc();


  out         <- 1-as.matrix(numTOM/denomTOM); rm(denomTOM); gc(); rm(numTOM); gc();
  diag(out)   <- 0
  return(out)
}
...
fastTOMwrapper <- function(X, expo = NULL, Method="pearson")
{
  ...
  if(is.null(expo)){A <- angularDist(X);}
  else{A <- X^expo; A[X < 0] <- 0}
  rm(X);gc();
...
{
  if(nrow(tEx) == 1){return(unlist(tEx))}
  tEx <- as.matrix(tEx)
  pc1 <- prcomp(t(tEx))$x[,1]
  if(sum(cor(t(tEx), pc1) < 0, na.rm = T) / nrow(tEx) > .5){pc1 <- -pc1}
  return(pc1)
}
...
cutreeHybridWrapper <- function(d, quantCut = 0.75)
{
  ...
  modules   <- dynamicTreeCut::cutreeHybrid(dendro = dend, distM = d, cutHeight = cutHeight,minClusterSize = 5, pamStage = T, pamRespectsDendro = F)
  return(modules)
}
...
# we use kurtosis
dropModuels <- function(eigenGenes, Kurts=NULL, corCut=.95, verbose=F)
{

  eigen_Cors <- cor(t(eigenGenes)); diag(eigen_Cors) <- 0
  while(any(eigen_Cors > corCut))
  {
    ...
    for(i in 1:(nrow(eigen_Cors)))
    {
      for(j in (i+1):nrow(eigen_Cors))
      {
        ...
        {
          if(verbose){ print(paste("threshold",corCut,"exceeded for mods",rownames(eigenGenes)[i],",",rownames(eigenGenes)[j])) }
          if(is.null(Kurts))
          {
            removeInd <- j
          }else
          {
            if(Kurts[i] < Kurts[j]){removeInd <- i}else{removeInd <- j}
  ...
  eigen_Cors <- cor(t(eigenGenes)); diag(eigen_Cors) <- 0
  print(paste("eigegenes trimmed to",nrow(eigenGenes),"due to correlation >", corCut,"max eigenCor =", max(signif(eigen_Cors,2))))
  return(eigenGenes)
}
...
simpWGCNAsubNet <- function(tEx,expo=6,Method = "pearson", n = 15, minMods=5, corCut=.6)
{
  print(paste("Computing",nrow(tEx), "x", nrow(tEx) ,"TOM distance for subset of genes with higher variance"))

  TOMd    <- fastTOMwrapper(tEx, expo = expo,Method = Method); gc();
  mods    <- cutreeHybridWrapper(TOMd)$labels
  modSz   <- table(mods)
  if(sum(modSz >= n) < minMods){ retMods  <- names( modSz[rank(-modSz) <= minMods]);
  } else{ retMods  <- names(modSz[modSz >= n]); }

  retMods    <- retMods[retMods != "0"]; # omit the catch all module for genes that do not belong to a community
  print(paste("number of modules found is",length(retMods)))

  eigenGenes <- as.matrix(plyr::ldply(.data =  retMods, .fun = function(m){eg <- calcEigenGene(tEx[mods == m,]); return(eg)}))
  rownames(eigenGenes) <- retMods

  eigenGenes <- dropModuels(eigenGenes = eigenGenes, Kurts = modSz[names(modSz) %in% retMods], corCut = corCut) # subsetting modSz to match eigenGenes
  tPc        <- prcomp(t(tEx))
  print(summary(tPc)$importance[2,1:3])
  corPC      <- cor(tPc$x[,1], t(eigenGenes))
  eigenGenes <- eigenGenes[order(abs(corPC),decreasing = T),]
...
#' @param pc_flag indicator. T (default) means to use the 1st principal component (corrected for direction). F uses the mean of scaled and centered top genes.
#'
...
#'
#' @examples
compute_eigengene_matrix <- function(ex,membership_matrix, cutoff = 5, pc_flag = T)
{
  ...
  tEigen <- plyr::aaply(as.matrix(meta),2,function(k)
  {
    ...
    if(all(tEx == 0)){return(rep(0, ncol(tEx)))}
    ...
    if(pc_flag == F){
      ...
      pc1 <- prcomp(t(tEx))$x[,1]
      if(cor(scaled_ave,pc1) < 0 ){pc1 <- -pc1}
      return(pc1)
    }
  })
}
